...
from utils.utils import get_kafka_producer, setup_database
...
def produce_smart_contract_txs(producer, transaction):
    ...
    """
    try: producer.send(topic=os.environ['TOPIC_BATCH_TX'], value=transaction)
...
def handle_smart_contract_txs(transactions):
    ...
    """
    interest_cols = ['blockNumber','timeStamp','from', 'to', 'isError','value','nonce','gasPrice','gasUsed','functionName']
    renamed_cols = ['block_number','timestamp','from','to','is_error','value','nonce','gasPrice','gasUsed','method']
...
if __name__ == '__main__':
    # python batch_historical_txs.py network=mainnet topic=batch_txs_contracts contract=0x7d2768dE32b0b80b7a3454c06BdAc94A69DDc7A9 start_date='2022-01-01 00:00:00'
    """
